...
public class FastCollinearPoints {
    ...
    public FastCollinearPoints(Point[] points) {
        ...
        Point oldLineEnd = sortedPoints[0];

        for (Point p : sortedPoints) {
            int count = 1;
            Point[] sortedBySlope = Arrays.copyOf(sortedPoints, sortedPoints.length);
            Arrays.sort(sortedBySlope, p.slopeOrder());

            for (int j = 0; j < sortedBySlope.length - 1; j++) {
                Point q1 = sortedBySlope[j];
                Point q2 = sortedBySlope[j + 1];

                if (p.compareTo(q1) <= 0) {
                    if (q1.slopeTo(p) == q2.slopeTo(p)) {
                        count++;
                        if (count >= 3 && j >= sortedBySlope.length - 2
                                && oldLineEnd.compareTo(q2) != 0) {
                            segmentsList.add(new LineSegment(p, q2));
                            oldLineEnd = q2;
                        }
                    }
                    else if (count >= 3 && oldLineEnd.compareTo(q1) != 0) {
                        segmentsList.add(new LineSegment(p, q1));
                        oldLineEnd = q1;
                        count = 1;
                    }
                    else {
